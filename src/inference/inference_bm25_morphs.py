"""
Inference script with wandb logging for MRC + BM25 Retrieval (Morpheme-based).

Usage:
    python -m src.inference.inference_bm25_morphs \
        --output_dir outputs/test_k100_morphs \
        ...
        --wandb_run_name "bm25_morphs_k100_test"
"""
...
from ..retrieval.retrieval_bm25_morphs import BM25RetrievalWithMetrics
...
def init_wandb(
    ...
    if run_name is None:
        run_name = f"bm25_morphs_k{data_args.top_k_retrieval}"
    ...
    wandb.init(
        ...
        config={
            ...
            "retrieval_method": "BM25-Morphs (Kiwi)",
...
def run_sparse_retrieval(
    ...
    # Initialize Kiwi for tokenization
    kiwi = Kiwi()
    def kiwi_tokenize(text):
        return [token.form for token in kiwi.tokenize(text)]

    # BM25Retrieval with metrics
    sparse_retriever = BM25RetrievalWithMetrics(
        tokenize_fn=kiwi_tokenize, data_path=data_path, context_path=context_path
    )
    sparse_retriever.get_sparse_embedding()

    # BM25는 현재 FAISS를 지원하지 않으므로 직접 검색
